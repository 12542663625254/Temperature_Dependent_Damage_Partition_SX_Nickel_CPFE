...
% Unlike the pure-fatigue (Stage 1) case, Dc is nonzero here, so all three
% terms can contribute. This breakdown shows, case by case, which term
% dominates the denominator (and therefore controls predicted life) -
% useful for judging whether d=2.9361 (cubic, very scale-sensitive) is
% being driven by genuine interaction or is an artifact of the imported
% M/B1 scaling when applied outside the 30/30 calibration range.
term_Dc    = Dc_vals.^a;                         % Dc^a term
term_Df    = Df_vals.^b;                         % Df^b term
term_cross = M*(Dc_vals.*Df_vals).^d;            % M*(Dc*Df)^d cross term
denom_sum  = term_Dc + term_Df + term_cross;     % full denominator
N_check    = 1 ./ denom_sum;                     % should equal pred_int exactly
pct_Dc     = 100*term_Dc./denom_sum;
pct_Df     = 100*term_Df./denom_sum;
pct_cross  = 100*term_cross./denom_sum;
...
fprintf('\n========= 60/0: DENOMINATOR TERM DECOMPOSITION =========\n');
fprintf(' N_cf = 1 / ( Dc^a  +  Df^b  +  M*(Dc*Df)^d )\n');
fprintf('----------------------------------------------------------\n');
disp(table((1:5)',Dc_vals',Df_vals',term_Dc',term_Df',term_cross',denom_sum',N_check', ...
    'VariableNames',{'Case','Dc','Df','Dc_a_term','Df_b_term','Cross_term','Denom_sum','N_check'}));
fprintf('\n--- Percent contribution of each term to the denominator ---\n');
disp(table((1:5)',pct_Dc',pct_Df',pct_cross', ...
    'VariableNames',{'Case','Pct_Dc_a','Pct_Df_b','Pct_Cross'}));
fprintf('=============================================================\n');
